@extends('layout.app')
@section('title', 'تعديل ورشة داخلية ')
@section('style')
...
@section('body')
      ...
      <section class="section">
          <div class="section-header">
            <h1>تعديل ورشة داخلية </h1>
            
          </div>
          <div class="section-body">
            @if (session('EditWorkshop'))
                <div class="alert alert-success">
                    {{ session('EditWorkshop') }}
                </div>
            @endif
            ...
            <div class="row mt-sm-4">
              <div class="col-12 col-md-12 col-lg-12">
                <div class="card">
                  <form method="post" class="needs-validation" novalidate="" action="/workshops/{{ $workshop->id }}"  enctype="multipart/form-data"  id="myForm" dir="rtl">
                    @csrf
                    ...
                    <div class="card-body">
                      <div class="row">         
                          ...
                          <div class="form-group col-md-4 col-12">
                            ...
                            <input type="text" class="form-control"  name="code"   value="{{$workshop->code }}"  required="">
                       ...
                       <strong  class="float-right" style="color: red;">{{ $errors->first('code') }}</strong>
                        ...
                        @foreach ( $locations as $location)
                            <option value=" {{ $location->id }}"{{ ($workshop->location == $location->id ? "selected":"") }}> {{ $location->name }}</option>
                        @endforeach                   
                      </select>       
  ...
  <div class="row">         
                        ...
                        @foreach ( $companies as $company)
                            <option value=" {{ $company->id }}"{{ ($workshop->company == $company->id ? "selected":"") }}> {{ $company->name_ar }}</option>
                        @endforeach                   
                      </select>       
                     ...
                     <div class="form-group col-md-4 col-12">
                        ...
                        @foreach ( $departments as $department)
                            <option value="{{ $department->id }}"{{ ($workshop->department == $department->id ? "selected":"") }}> {{ $department->name }}</option>
                        @endforeach                   
                      </select>       
 ...
 <div class="form-group col-md-4 col-12">
                            ...
                            <input type="text" class="form-control"  name="notes"   value="{{ $workshop->notes}}"  >
                          </div>
