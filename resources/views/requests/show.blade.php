@extends('layout.app')
@section('title', ' تصريح خروج مُـــعده ')
@section('style')
          ...
          <div class="section-body">
            ...
            <div class="row mt-sm-4">
              <div class="col-12 col-md-12 col-lg-12">
                <div class="card">
                  <form method="post" class="needs-validation" novalidate="" action="/requests/out"  enctype="multipart/form-data"  id="myForm" dir="rtl">
                    @csrf
                    <input type="hidden"  name="request_id" value="{{ $request->id }}"  readonly   required="">
                    ...
                    <select class="form-control select2" name='driver[]' multiple required >
                       ...
                       <strong  class="float-right" style="color: red;">{{ $errors->first('driver') }}</strong>
                           ...
                           <div class="form-group col-md-4 col-12">
                            <label  class="float-right"> موقع العمل </label>
                            <select class="form-control select2" name='location' required >
                        <option disabled selected value="">أختر موقع العمل </option>
                        @foreach ( $locations as $location)
                            <option value=" {{ $location->id }}"> {{ $location->name }}</option>
                        @endforeach                   
                      </select>       
                        <div class="invalid-feedback"> ادخل موقع العمل </div>
